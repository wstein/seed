...
for grammar in Hello Expr Ctx JSON ModesLexer Modes Calc MiscLexer Misc MoreLexer More; do
  cp "$grammars/$grammar.g4" "$workdir/"
done
cp scripts/ParseDump.java "$workdir/"

( cd "$workdir" && java -jar "$ANTLR_JAR" -Dlanguage=Java Hello.g4 Expr.g4 Ctx.g4 JSON.g4 ModesLexer.g4 Modes.g4 Calc.g4 MiscLexer.g4 Misc.g4 MoreLexer.g4 More.g4 )
...
for entry in "hello:Hello:greeting" "expr:Expr:prog" "ctx_a:Ctx:s" "ctx_b:Ctx:s" "json:JSON:json" "modes:Modes:prog" "calc:Calc:prog" "misc:Misc:prog" "more:More:prog"; do
  name="${entry%%:*}"
  rest="${entry#*:}"
  grammar="${rest%%:*}"
  start="${rest##*:}"
