...
for grammar in Hello Expr Ctx JSON ModesLexer Modes Calc; do
  cp "$grammars/$grammar.g4" "$workdir/"
done
cp scripts/ParseDump.java "$workdir/"

( cd "$workdir" && java -jar "$ANTLR_JAR" -Dlanguage=Java Hello.g4 Expr.g4 Ctx.g4 JSON.g4 ModesLexer.g4 Modes.g4 Calc.g4 )
...
for entry in "hello:Hello:greeting" "expr:Expr:prog" "ctx_a:Ctx:s" "ctx_b:Ctx:s" "json:JSON:json" "modes:Modes:prog" "calc:Calc:prog"; do
  name="${entry%%:*}"
  rest="${entry#*:}"
  grammar="${rest%%:*}"
  start="${rest##*:}"
